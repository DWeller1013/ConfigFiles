...
static _Bool cfc_has_mode(void);
static inline void free_cptext(char *const *const cptext);
static _Bool is_nearest_active(void);
static void reposition_match(compl_T *match);
static int ins_compl_add(char *const str, int len, char *const fname, char *const *const cptext, const _Bool cptext_allocated, typval_T *user_data, const Direction cdir, int flags_arg, const _Bool adup, const int *user_hl, const int score) FUNC_ATTR_NONNULL_ARG(1);
...
static void ins_compl_insert_bytes(char *p, int len) FUNC_ATTR_NONNULL_ALL;
static _Bool ins_compl_has_multiple(void);
...
static void ins_compl_dictionaries(char *dict_start, char *pat, int flags, _Bool thesaurus);
static int thesaurus_add_words_in_line(char *fname, char **buf_arg, int dir, const char *skip_word);
static void ins_compl_files(int count, char **files, _Bool thesaurus, int flags, regmatch_T *regmatch, char *buf, Direction *dir) FUNC_ATTR_NONNULL_ARG(2, 7);
static void ins_compl_free(void);
static _Bool ins_compl_has_preinsert(void);
static _Bool ins_compl_refresh_always(void) FUNC_ATTR_PURE;
static _Bool ins_compl_need_restart(void) FUNC_ATTR_PURE;
static void ins_compl_new_leader(void);
static int get_compl_len(void);
static void ins_compl_restart(void);
static void ins_compl_set_original_text(char *str, size_t len) FUNC_ATTR_NONNULL_ALL;
...
static void fill_complete_info_dict(dict_T *di, compl_T *match, _Bool add_match);
static void get_complete_info(list_T *what_list, dict_T *retdict);
static _Bool thesaurus_func_complete(int type);
static int process_next_cpt_value(ins_compl_next_state_T *st, int *compl_type_arg, pos_T *start_match_pos, _Bool fuzzy_collect);
