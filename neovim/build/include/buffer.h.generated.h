...
DLLEXPORT _Bool buf_ensure_loaded(buf_T *buf);
DLLEXPORT int open_buffer(_Bool read_stdin, exarg_T *eap, int flags_arg);
...
DLLEXPORT void set_curbuf(buf_T *buf, int action, _Bool update_jumplist);
...
DLLEXPORT void free_buf_options(buf_T *buf, _Bool free_p_ff);
...
DLLEXPORT void fileinfo(int fullname, int shorthelp, _Bool dont_truncate);
...
DLLEXPORT _Bool append_arg_number(win_T *wp, char *buf, int buflen) FUNC_ATTR_NONNULL_ALL;
...
DLLEXPORT _Bool buf_is_empty(buf_T *buf);
