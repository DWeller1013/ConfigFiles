// IWYU pragma: private, include "nvim/fold.h"
...
DLLEXPORT _Bool hasFolding(win_T *win, linenr_T lnum, linenr_T *firstp, linenr_T *lastp);
...
DLLEXPORT _Bool foldmethodIsManual(win_T *wp);
DLLEXPORT _Bool foldmethodIsIndent(win_T *wp);
DLLEXPORT _Bool foldmethodIsExpr(win_T *wp);
DLLEXPORT _Bool foldmethodIsMarker(win_T *wp);
DLLEXPORT _Bool foldmethodIsSyntax(win_T *wp);
DLLEXPORT _Bool foldmethodIsDiff(win_T *wp);
DLLEXPORT void closeFold(pos_T pos, int count);
DLLEXPORT void closeFoldRecurse(pos_T pos);
DLLEXPORT void opFoldRange(pos_T firstpos, pos_T lastpos, int opening, int recurse, _Bool had_visual);
DLLEXPORT void openFold(pos_T pos, int count);
...
DLLEXPORT int foldMoveTo(const _Bool updown, const int dir, const int count);
...
DLLEXPORT void foldAdjustCursor(win_T *wp);
...
DLLEXPORT char *get_foldtext(win_T *wp, linenr_T lnum, linenr_T lnume, foldinfo_T foldinfo, char *buf, VirtText *vt) FUNC_ATTR_NONNULL_ALL;
