...
DLLEXPORT int utf_ptr2cells(const char *p_in);
...
DLLEXPORT _Bool utf_iscomposing_first(int c);
DLLEXPORT _Bool utf_composinglike(const char *p1, const char *p2, GraphemeState *state) FUNC_ATTR_NONNULL_ARG(1, 2);
DLLEXPORT _Bool utf_iscomposing(int c1, int c2, GraphemeState *state);
DLLEXPORT schar_T utfc_ptr2schar(const char *p, int *firstc) FUNC_ATTR_NONNULL_ALL;
DLLEXPORT schar_T utfc_ptrlen2schar(const char *p, int len, int *firstc) FUNC_ATTR_NONNULL_ALL;
...
DLLEXPORT _Bool utf_iscomposing_legacy(int c);
DLLEXPORT _Bool utf_printable(int c) FUNC_ATTR_CONST;
DLLEXPORT int utf_class(const int c);
DLLEXPORT int utf_class_tab(const int c, const uint64_t *const chartab) FUNC_ATTR_PURE;
DLLEXPORT _Bool utf_ambiguous_width(const char *p);
...
DLLEXPORT int mb_off_next(const char *base, const char *p);
DLLEXPORT CharBoundsOff utf_cp_bounds_len(char const *base, char const *p_in, int p_len) FUNC_ATTR_PURE FUNC_ATTR_NONNULL_ALL;
DLLEXPORT CharBoundsOff utf_cp_bounds(char const *base, char const *p_in) FUNC_ATTR_PURE FUNC_ATTR_NONNULL_ALL;
...
DLLEXPORT void f_iconv(typval_T *argvars, typval_T *rettv, EvalFuncData fptr);
...
DLLEXPORT char *get_encoding_name(expand_T *xp FUNC_ATTR_UNUSED, int idx);
DLLEXPORT int mb_strcmp_ic(_Bool ic, const char *s1, const char *s2) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT;
#include "nvim/func_attr.h"
