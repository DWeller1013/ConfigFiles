...
#  endif
#endif
DLLEXPORT char *os_getenv(const char *name) FUNC_ATTR_NONNULL_ALL;
DLLEXPORT char *os_getenv_noalloc(const char *name) FUNC_ATTR_NONNULL_ALL;
DLLEXPORT _Bool os_env_exists(const char *name, _Bool nonempty) FUNC_ATTR_NONNULL_ALL;
...
DLLEXPORT const char *os_homedir(void);
...
DLLEXPORT void expand_env_esc(const char *restrict srcp, char *restrict dst, int dstlen, _Bool esc, _Bool one, char *prefix) FUNC_ATTR_NONNULL_ARG(1, 2);
